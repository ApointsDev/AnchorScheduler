...
export class TaskStore {
    ...
    async addTask(userId: string, task: Task, boundaryConflict?: boolean, allowConflict: boolean = true): Promise<void> {
        ...
        if (!task.quadrant) {
            task.quadrant = quadrantFromAxes(
        ...
        await this.db.run(
            `INSERT INTO tasks (id, userId, name, description, dueDate, startTime, endTime, location, completed, pushedToMSTodo, body, attendees, recurrenceRule, parentTaskId, importance, scheduleType, quadrant, completedAt, importanceScore, urgencyScore, visibility, authorizedUserIds, blockedUserIds)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [task.id, userId, task.name, task.description, task.dueDate, task.startTime, task.endTime, task.location, task.completed ? 1 : 0, task.pushedToMSTodo ? 1 : 0, task.body, task.attendees ? JSON.stringify(task.attendees) : null, task.recurrenceRule || null, task.parentTaskId || null, task.importance || "normal", task.scheduleType || "single", task.quadrant || null, completedAt, axes.importanceScore, axes.urgencyScore, task.visibility || "private", task.authorizedUserIds ? JSON.stringify(task.authorizedUserIds) : null, task.blockedUserIds ? JSON.stringify(task.blockedUserIds) : null],
        );
    ...
    async updateTask(task: Task, boundaryConflict?: boolean, allowConflict: boolean = false): Promise<void> {
        ...
        const wasCompleted = !!(row && (row.completed === 1 || row.completed === true));
        const completedAt = this.resolveCompletedAt(
            wasCompleted,
            !!task.completed,
            row?.completedAt,
        ...
        await this.db.run(
            `UPDATE tasks SET name = ?, description = ?, dueDate = ?, startTime = ?, endTime = ?, location = ?, completed = ?, pushedToMSTodo = ?, body = ?, attendees = ?, recurrenceRule = ?, parentTaskId = ?, importance = ?, scheduleType = ?, quadrant = ?, completedAt = ?, importanceScore = ?, urgencyScore = ?, visibility = ?, authorizedUserIds = ?, blockedUserIds = ?, updatedAt = CURRENT_TIMESTAMP WHERE id = ?`,
            [task.name, task.description, task.dueDate, task.startTime, task.endTime, task.location, task.completed ? 1 : 0, task.pushedToMSTodo ? 1 : 0, task.body, task.attendees ? JSON.stringify(task.attendees) : null, task.recurrenceRule || null, task.parentTaskId || null, task.importance || "normal", task.scheduleType || "single", task.quadrant || null, completedAt, task.importanceScore ?? null, task.urgencyScore ?? null, task.visibility || "private", task.authorizedUserIds ? JSON.stringify(task.authorizedUserIds) : null, task.blockedUserIds ? JSON.stringify(task.blockedUserIds) : null, task.id],
        );
     ...
     */
    async getVisibleTasksByUserId(targetUserId: string, viewerUserId: string): Promise<Task[]> {
