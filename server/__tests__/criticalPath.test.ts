import { CriticalPathAnalysis } from '../Services/Algorithms/CriticalPathAnalysis';
import { CriticalPathInput } from '../Services/Algorithms/types';

describe('CriticalPathAnalysis', () => {
  let analyzer: CriticalPathAnalysis;
  const startDate = new Date('2023-01-01T09:00:00Z');

  beforeEach(() => {
    analyzer = new CriticalPathAnalysis();
  });

  test('should identify critical path in a simple linear sequence', () => {
    const input: CriticalPathInput = {
      startDate,
      tasks: [
        { id: 'A', duration: 60, dependencies: [] },
        { id: 'B', duration: 30, dependencies: ['A'] },
        { id: 'C', duration: 45, dependencies: ['B'] }
      ]
    };
  ...
  test('should identify critical path in a branching graph', () => {
    // A(10) -> B(20) -> D(10) = 40
    ...
    const input: CriticalPathInput = {
      ...
      tasks: [
        ...
        { id: 'C', duration: 50, dependencies: ['A'] },
        { id: 'D', duration: 10, dependencies: ['B', 'C'] }
      ]
    };
  ...
  test('should correctly calculate PERT expected duration', () => {
    const input: CriticalPathInput = {
      startDate,
      tasks: [
        { 
          id: 'A', 
          duration: 0, // Should be ignored
          dependencies: [],
          optimistic: 10,
          mostLikely: 20,
          pessimistic: 60
  ...
  test('should throw error for cyclic dependencies', () => {
    const input: CriticalPathInput = {
      startDate,
      tasks: [
        { id: 'A', duration: 10, dependencies: ['B'] },
        { id: 'B', duration: 10, dependencies: ['A'] }
      ]
    };

    expect(() => analyzer.analyze(input)).toThrow("Cycle detected");
  });

  test('should handle complex dependencies correctly', () => {
    //      /-> B(10) -\
    // A(10)            -> D(20) -> F(10)
    //      \-> C(20) -/
    //           \-> E(5) -> G(10)
    ...
    const input: CriticalPathInput = {
      ...
      tasks: [
        { id: 'A', duration: 10, dependencies: [] },
        { id: 'B', duration: 10, dependencies: ['A'] },
        { id: 'C', duration: 20, dependencies: ['A'] },
        { id: 'D', duration: 20, dependencies: ['B', 'C'] },
        { id: 'E', duration: 5, dependencies: ['C'] },
        { id: 'F', duration: 10, dependencies: ['D'] },
        { id: 'G', duration: 10, dependencies: ['E'] }
      ]
    };
