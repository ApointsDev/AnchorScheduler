...
export function isScheduleType(value: unknown): value is ScheduleType {
    return (
        typeof value === "string" &&
        (scheduleTypeValues as readonly string[]).includes(value)
    );
}

export function parseRecurrenceRuleInput(
    rule: unknown,
    ...
    if (!maybeRule.freq || !recurrenceFreqValues.includes(maybeRule.freq))
        return undefined;
