...
export function clampPreferredHour(h: number): number {
...
export function computeNextSyncAt(
    from: Date | string | moment.Moment,
    intervalHours: number,
    preferredHour: number,
    jitterMinutes = 0,
): string {
    const interval = clampIntervalHours(intervalHours);
    const hour = clampPreferredHour(preferredHour);
    const jitter = Math.max(0, Math.min(59, Math.floor(jitterMinutes)));

    const base = moment.tz(from, TZ).add(interval, "hours");
    // 冷却结束后，落到不早于 base 的 preferredHour:00 + jitter
    let candidate = base.clone().hour(hour).minute(jitter).second(0).millisecond(0);
...
export function jitterMinutesForUser(userId: string): number {
    let h = 0;
...
export function isDue(nextSyncAt: string | undefined | null, now = new Date()): boolean {
    if (!nextSyncAt) return true;
    const t = Date.parse(nextSyncAt);
