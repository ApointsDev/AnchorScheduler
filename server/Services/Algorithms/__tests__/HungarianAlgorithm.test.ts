...
describe('Hungarian Algorithm', () => {
  ...
  test('should handle unbalanced input (Tasks < Slots)', () => {
    // T1 -> S1 (1), S2 (100)
    // S3 unused
    const input: BipartiteMatchingInput = {
      leftNodes: ['T1'],
  ...
  test('should handle Infinity costs (forbidden edges)', () => {
    // T1 -> S1 (Inf), S2 (1)
    // T2 -> S1 (1), S2 (Inf)
    const input: BipartiteMatchingInput = {
      leftNodes: ['T1', 'T2'],
  ...
  test('should respect required matches', () => {
    // T1 -> S1 (10), S2 (1)
    // T2 -> S1 (1), S2 (10)
